---
title: 'Kobe Bryant Make His Next Shot: Linear Discriminant Analysis and Logistic
  Regression '
author:
- Paul Adams
- Reannan McDaniel
...
output:
  ...
  html_document:
    df_print: paged
abstract: This project investigates the correlation between multiple potential explanatory
  variables and Kobe Bryant's ability to make a shot while playing for the NBA team
  Los Angeles Lakers using data gathered from `1996`-`2015`.
---

```{r Setup and Loading Packages, echo=F, include=F}
library(pacman)
...
```{r Data Transformations Phase One, echo=F}
...
```{r Transformations Phase Two, echo=F}
...
## Multicollinearity Among Predictor Variables Phase One: Correlation Plot for Visual Data Exploration
...
## Multicollinearity Among Predictor Variables Phase Two: Correlation Matrix
...
```{r Correlation Matrix for Quantitative Data, echo=F, include=T}
...
### Bartlett's Test:
```{r Quadratic Discriminant Analysis: Test Identification, echo=F, include=T}
...
```{r Quadratic Discriminant Analysis: Test Production, echo=F}
...
```{r Variable Boundaries Models, echo=F}
...
### shot_made_flag ~ shot_distance + attendance + combined_shot_type + arena_temp + game_event_id + seconds_remaining + shot_type +
      game_date + minutes_remaining + loc_y + shot_id
```{r Logistic Regression: Forward Selection, echo=F, include=T}
#### Forward Selection
...
### shot_made_flag ~ combined_shot_type + game_event_id + loc_y + minutes_remaining + seconds_remaining + shot_distance + shot_type + game_date + shot_id + attendance + arena_temp
```{r Logistic Regression: Backward Elimination, echo=F, include=T}
# Backward Elimination
...
### shot_made_flag ~ combined_shot_type + game_event_id + loc_y + minutes_remaining + seconds_remaining + shot_distance + shot_type + game_date +
shot_id + attendance + arena_temp
```{r Logistic Regression: Stepwise Regression, echo=F, include=T}
# Stepwise Regression
