...
df.qda.filtered <- suppressWarnings(predict(qda.filtered, newdata = df.numeric))

#droplevels(subDF.Test.numeric$shot_made_flag) class(dfTest.numeric$shot_made_flag)
confusion_matrix_results <- confusionMatrix(table(df.qda.filtered, df.numeric$shot_made_flag_predicted))

```

## **Quadratic Discriminant Analysis Misclassification Rate**
```{r Confusion Matrix Production, echo=F, include=T, warning=F}
...
logloss.subDF.Train.numeric <- subDF.Train.numeric
logloss.subDF.Train.numeric$shot_made_flag <- ifelse(logloss.subDF.Train.numeric$shot_made_flag == "made", 1, 0)
logloss.test_pred.qda.filtered <- test_pred.qda.filtered
logloss.test_pred.qda.filtered <- ifelse(logloss.test_pred.qda.filtered == "made", 1, 0)

loglossTraining = logloss.subDF.Train.numeric %>% mutate(logloss = logloss.subDF.Train.numeric$shot_made_flag * log(1-logloss.test_pred.qda.filtered) + (1-logloss.subDF.Train.numeric$shot_made_flag)*log(1-logloss.test_pred.qda.filtered))
...
confusion_matrix_results.preds <- confusionMatrix(table(test_pred.qda.filtered, df.numeric.preds$shot_made_flag))
