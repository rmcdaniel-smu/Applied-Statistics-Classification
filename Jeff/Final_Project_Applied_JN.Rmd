...
### As with the Qudratic Discriminant Analysis: External Cross-Valdiation and Model Development section, confusion matricies are used to assess model performance where we will be focusing on `Accuracy`, `Precision`, `Sensitivity`, `Specificity`, `Misclassification Rate`, and `AIC`.  `Accuracy` is the number of correct predicitons divided by total observations.  `Precision` is number of correctly classified positive observations over total overservations.  `Sensitivity` is a model's ability to correctly identify positive observations divided by all positive classificaitons. 
